// We have forked: https://github.com/go-openapi/runtime/blob/4559f9fe8183f36685514029366d833e3a960292/client/runtime.go
// We did this since we need to authentication to happen after the building of the HTTP request.
...
import (
	...
	"encoding/pem"
	"fmt"
	forked "github.com/go-openapi/runtime/client"
	"io/ioutil"
	"mime"
	"net/http"
	"net/http/httputil"
...
func TLSClientAuth(opts TLSClientOptions) (*tls.Config, error) {
	// create eschr_client tls config
	cfg := &tls.Config{}

	// load eschr_client cert if specified
	if opts.Certificate != "" {
		cert, err := tls.LoadX509KeyPair(opts.Certificate, opts.Key)
		if err != nil {
			return nil, fmt.Errorf("tls eschr_client cert: %v", err)
		}
		...
		case *ecdsa.PrivateKey:
			...
			if err != nil {
				return nil, fmt.Errorf("tls eschr_client priv key: %v", err)
			}
		default:
			return nil, fmt.Errorf("tls eschr_client priv key: unsupported key type")
		}
	...
	if opts.LoadedCA != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("tls eschr_client ca: %v", err)
		}
		caCertPool := x509.NewCertPool()
...
// Runtime represents an API eschr_client that uses the transport
...
func New(host, basePath string, schemes []string) *Runtime {
	...
	rt.Consumers = map[string]runtime.Consumer{
		...
		"application/gzip":  runtime.ByteStreamConsumer(),
	}
	rt.Producers = map[string]runtime.Producer{
		...
		"application/gzip":  runtime.ByteStreamProducer(),
	}
...
// So instead it's provided as a http eschr_client middleware that can be used to override
// any request.
func (r *Runtime) EnableConnectionReuse() {
	if r.client == nil {
		r.Transport = forked.KeepAliveTransport(
			transportOrDefault(r.Transport, http.DefaultTransport),
		)
		return
	}

	r.client.Transport = forked.KeepAliveTransport(
...
func (r *Runtime) Submit(operation *runtime.ClientOperation) (interface{}, error) {
	...
	// Hack: put also header so auth will get it
	req, err := request.buildHTTP(cmt, r.BasePath, r.Producers, r.Formats, auth, r.Host)
	if err != nil {
		return nil, err
	}
...
// It ensures that eschr_client and middlewares have the set debug level.
...
// It ensures that eschr_client and middlewares use the same logger.
